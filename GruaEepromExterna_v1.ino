...
-MODIFICADO para usar EEPROM externa I2C*/
...
// Dirección I2C de la EEPROM externa (típicamente 0x50)
...
// Funciones para EEPROM externa I2C
void writeEEPROM(int address, byte data) {
  // Leer el valor actual
  byte current = readEEPROM(address);
...
// Función para leer enteros de EEPROM externa
...
// Función para leer unsigned int de EEPROM externa
...
void setup() {
    ...
    Serial.begin(115200);
    Wire.begin(); // Inicializar I2C para EEPROM externa
    ...
    // Leer variables de la EEPROM externa
    INICIO = getEEPROM(45);
    if (INICIO != 35) {
        putEEPROMUint(1, 0);
        putEEPROMUint(5, 0);
  ...
  COIN = getEEPROMUint(1);
  CONTSALIDA = getEEPROMUint(5);
  BANK = getEEPROM(9);
  PAGO = getEEPROM(13);
  TIEMPO = getEEPROM(17);
  FUERZA = getEEPROM(21);
  PJFIJO = getEEPROMUint(25);
  PPFIJO = getEEPROMUint(29);
  BARRERAAUX2 = getEEPROM(33);
  GRUADISPLAY = getEEPROM(37);
  TIEMPO5 = getEEPROM(41);
  
  if (digitalRead(DATO3) == LOW){ 
    lcd.clear ();lcd.setCursor(0,0);lcd.print("CONT.FIJOS"); lcd.setCursor(0,1);lcd.print("PJ:"); 
...
void loop() {
  ...
  int dato4, auxtbarrera = 0;
    ...
    while (digitalRead(EPINZA) == LOW && AUX < 5) {
        ...
        leecoin();
          ...
          if (PJFIJO != prevPJFIJO || PPFIJO != prevPPFIJO || BANK != prevBANK) {
              dato1 = PAGO; dato2 = PJFIJO; dato3 = PPFIJO; dato4 = BANK;
    ...
    // Guardar los valores actualizados en la EEPROM externa
    putEEPROM(9, BANK);
