...
const char* ssid = "SLa1000-523F";
const char* password = "SLa10003C40";
...
const long wifiReconnectTimeout = 10000; 
...
void connectToWiFi() {
    // Si ya está conectado, limpiar flags y salir
    ...
    // Si NO estamos intentando reconectar, INICIAR el proceso
    if (!intentandoReconectar) {
        //Serial.println("Iniciando reconexión WiFi...");
        WiFi.disconnect(true);
        delay(100);  // Delay mínimo necesario
        WiFi.begin(ssid, password);
        wifiReconnectStart = millis();
        intentandoReconectar = true;
        //Serial.print("Conectando");
        return;  // ← SALE INMEDIATAMENTE, no bloquea
    }
    
    // Si YA estamos intentando, verificar el progreso
    unsigned long tiempoTranscurrido = millis() - wifiReconnectStart;
    
    // Mostrar progreso cada segundo (opcional)
    ...
    if (tiempoTranscurrido > wifiReconnectTimeout) {
        //Serial.println("\n✗ Timeout WiFi. Reintentando más tarde...");
        intentandoReconectar = false;
        WiFi.disconnect(true);
        return;
    }
    
    // Si se conectó durante el intento
    if (WiFi.status() == WL_CONNECTED) {
        //Serial.println("\n✓ WiFi Conectado!");
        //Serial.print("IP: ");
        //Serial.println(WiFi.localIP());
    ...
    // Si no está conectado aún, no hacer nada (seguir esperando)
    // La próxima llamada verificará de nuevo
}
...
void reconnectMQTT() {
    ...
    // ⭐ RATE LIMITING: No intentar muy seguido
    unsigned long now = millis();
    if (now - lastMqttAttempt < mqttReconnectInterval) {
        return;  // ← SALE sin intentar
    }
    lastMqttAttempt = now;
    ...
    if (client.connect(clientId.c_str(), 
        ...
        client.publish("maquinas/status", "online", true);
    } //else {
...
void loop() {
    // ===========================================================
    // MANEJO DE WIFI Y MQTT AL INICIO DEL LOOP
    // ===========================================================
    if (WiFi.status() == WL_CONNECTED) {
        if (!client.connected()) {
            reconnectMQTT();  // ← Con rate limiting interno
        }
        if (client.connected()) {
            client.loop();
        }
    } else if (intentandoReconectar) {
        connectToWiFi();
    }
    ...
    // ============================================================================
    // BUCLE DE ESPERA - CORREGIDO
    // ============================================================================
    while (digitalRead(EPINZA) == LOW && AUX < 5) {
        
        // ---- MANEJO DE WIFI Y MQTT DENTRO DEL WHILE ----
        if (WiFi.status() == WL_CONNECTED) {
            // SOLO intentar reconectar si no está conectado
            if (!client.connected()) {
                reconnectMQTT();  // ← Con rate limiting, no intenta cada ciclo
            }
            // Solo hacer loop si está conectado
            if (client.connected()) {
                client.loop();
            }
        } else if (intentandoReconectar) {
        ...
        // Verificación periódica de WiFi (cada 30 seg)
        if (millis() - lastWifiCheck > wifiCheckInterval) {
            if (WiFi.status() != WL_CONNECTED && !intentandoReconectar) {
                connectToWiFi();
            }
        ...
        // DETECCIÓN DE CAMBIOS Y ENVÍO MQTT (con validación)
        ...
        // Detección de botón de programación
